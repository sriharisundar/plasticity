                                                 ...
                                                 unsigned int quadPtID)
{
	...
	for (unsigned int i = 0;i<6;i++) {
		for (unsigned int j = 0;j<6;j++) {
			elasticStiffnessMatrix[i][j] = userInputs.elasticStiffness[i][j];
		}
	}
    ...
    unsigned int n_PA=0;	// Number of active slip systems
		...
		for (unsigned int i = 0;i < n_slip_systems;i++) {
			...
			if (i > userInputs.numSlipSystems - 1) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//	resolved_shear_tau_trial = resolved_shear_tau;
        ...
        for(unsigned int i=0;i<userInputs.numTwinSystems;i++){
            h_beta(userInputs.numSlipSystems+i)=userInputs.initialHardeningModulusTwin[i]*pow((1-s_beta(userInputs.numSlipSystems+i)/userInputs.saturationStressTwin[i]),userInputs.powerLawExponentTwin[i]);
        }
           ...
           temp1.reinit(dim,dim);temp1=0.0;
            ...
            for (unsigned int i=0;i<userInputs.numSlipSystems;i++){//
                     ...
                     if(s_alpha_tau(i)>(userInputs.saturationStress[i])){
                        s_alpha_tau(i)=userInputs.saturationStress[i];
 ////abort();//
      ...
      for (unsigned int i=0;i<userInputs.numTwinSystems;i++){//
                 ...
                 if(s_alpha_tau(userInputs.numSlipSystems+i)>(userInputs.saturationStressTwin[i])){
                 s_alpha_tau(userInputs.numSlipSystems+i)=userInputs.saturationStressTwin[i];
 ...
 //abort();
  ...
  P_tau.equ(det_F_tau, P_tau);
         ...
         for (unsigned int i=0;i<userInputs.numTwinSystems;i++){
            twinfraction_iter[cellID][quadPtID][i]=twinfraction_conv[cellID][quadPtID][i]+x_beta_old[i+userInputs.numSlipSystems]/userInputs.twinShear;
            twinfraction_iter_Twin[cellID][quadPtID][i] = twinfraction_conv_Twin[cellID][quadPtID][i] + x_beta_old[i + userInputs.numSlipSystems] / userInputs.twinShear;
}
